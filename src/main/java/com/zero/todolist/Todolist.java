...
public class Todolist {
    ...
    /**
     * @param content 添加待办事项内容
     */
    public void addItem(String content) {
        Todolistitem item = new Todolistitem(content);
        item.setId(items.size() + 1);  // 简单分配ID
        items.add(item);
        System.out.println("已添加待办事项: " + content);
    }

    /**
     * @param content 添加特殊待办事项内容
     */
    public void addSpecialItem(String content, int deadline) {
        Specialtodolistitem item = new Specialtodolistitem(content, deadline);
        item.setId(items.size() + 1);  // 简单分配ID
        items.add(item);
        System.out.println("已添加截止日期为" + item.getStrdeadline() + "的特殊待办事项: " + content);
    }
    ...
    public void toggleItemState(int index) {
        if (index >= 0 && index < items.size()) {
            ...
            System.out.println("已切换事项" + items.get(index).getContent() + "到" + items.get(index).isCompleted());
        } else {
    ...
    public void removeItem(int index) {
        if (index >= 0 && index < items.size()) {
            ...
            System.out.println("已删除待办事项: " + removed.getContent());
        } else {
            System.out.println("索引无效!");
        }
    }
     ...
     * 检查特殊待办事项是否超时
     * @param index 待办事项索引
     * @return 如果是特殊待办事项且超时则返回true，否则返回false
     */
    public boolean checkItem(int index){
        if (index >= 0 && index < items.size()) {
            Todolistitem item = items.get(index);
            if (item instanceof Specialtodolistitem) {
                boolean isTimeout = ((Specialtodolistitem) item).checkTimeout();
                System.out.println("已检查待办事项: " + item.getContent());
                return isTimeout;
            } else {
                System.out.println("该待办事项不是特殊待办事项，无法检查");
                return false;
            }
        } else {
            System.out.println("索引无效!");
